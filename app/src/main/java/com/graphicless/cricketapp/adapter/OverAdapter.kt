...
class OverAdapter(
    private val overs: Map<Int, List<Double>>,
    private val balls: List<FixtureOver.Data.Ball?>
) : RecyclerView.Adapter<OverAdapter.DataViewHolder>() {
    class DataViewHolder(view: View) : RecyclerView.ViewHolder(view) {

        val binding = ItemOverBinding.bind(view)
        fun bind(over: Int, item: List<Double>?, balls: List<FixtureOver.Data.Ball?>) {
            binding.tvOver.text =
                MyApplication.instance.getString(R.string.over).plus(" ").plus(over)
            val layoutManager =
                LinearLayoutManager(MyApplication.instance, LinearLayoutManager.HORIZONTAL, false)
    ...
    override fun onBindViewHolder(holder: DataViewHolder, position: Int) {
        ...
        holder.bind(overs.size - position, item, balls)
    }
}